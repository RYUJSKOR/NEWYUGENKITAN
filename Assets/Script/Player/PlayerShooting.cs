using System; using UnityEngine;  public abstract class PlayerShooting : IPlayerState {     private Player player;     private PlayerStateMachine playerStateMachine;     protected Shooting shooting; // 弾の発射処理クラス      protected float shootTimer = 0f;     protected float shootInterval = 0.15f;     public Vector3 shootingDirection = Vector3.zero;      Vector2 input;
    Vector3 direction;
    EightDirection shootDirEnum;      private bool wasGroundedLastFrame = true;      public virtual void Init(Player player, PlayerStateMachine playerStateMachine)     {         this.player = player;         this.playerStateMachine = playerStateMachine;         this.shooting = player.GetComponent<Shooting>();         shootTimer = 0f;
